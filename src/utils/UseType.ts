// * Reference https://github.com/ts-essentials/ts-essentials/blob/master/lib/types.ts
 ...
 */
export type Dictionary<T, K extends PropertyKey = string> = { [key in K]: T };
 ...
 */
export type SafeDictionary<T, K extends PropertyKey = string> = {
  [key in K]?: T;
};
...
export type StrictOmit<
  T extends AnyRecord,
  K extends keyof T
> = T extends AnyArray ? never : Omit<T, K>;
...
export type OmitProperties<T, V> = Omit<T, PickKeysByValue<T, V>>;
...
export type DeepPick<T, Filter extends DeepModify<T>> = T extends Builtin
  ...
  ? {
      ...
      [K in keyof T as K extends keyof Filter ? K : never]: Filter[K &
        keyof Filter] extends true
        ? T[K]
...
type _MergeN<T extends readonly any[], Result> = T extends readonly [
  infer Head,
  ...infer Tail
]
...
export type ElementOf<T extends readonly any[]> =
...
type IsEqualConsideringWritability<X, Y> = (<T>() => T extends X
  ? 1
...
export type Tail<T extends AnyArray> = T extends [any, ...infer Rest]
  ? Rest
  : never;
 ...
 */
export type PredicateType<T extends PredicateFunction> = T extends (
  target: any,
  ...rest: any[]
) => target is infer P
  ? P
...
export type ValueOfArray<A> = A extends Array<infer V> ? V : never;
